...
function getWeatherData(apiKey,locationKey) {
    ...
    axios.get(weatherUrl).then((response) => {
        ...
        if(weatherInfoObj.temperature.Maximum > 75){
            returnId('maximum').setAttribute('src', `https://developer.accuweather.com/sites/default/files/${weatherInfoObj.temperature.Maximum}-s.png`);
        }
        else {
            returnId('minimum').setAttribute('src', `https://developer.accuweather.com/sites/default/files/${weatherInfoObj.temperature.Minimum}-s.png`);  
        }
        
        returnId('maximum-desc').textContent = weatherInfoObj.temperature.Maximum;
        returnId('minimum-desc').textContent = weatherInfoObj.temperature.Minimum;
